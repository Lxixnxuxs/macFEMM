...
** on 2026-05-16 to support native macOS/Linux compilation.
** This is an altered Lua source file.
*/
    ...
    struct Capture* cap)
{
  ...
  if ((size_t)(cap->src_end - s) >= len && memcmp(cap->capture[l].init, s, len) == 0)
    return s + len;
  else
    return NULL;
}
...
static int push_captures(lua_State* L, struct Capture* cap)
{
  ...
  for (i = 0; i < cap->level; i++) {
    ...
    if (l == -1)
      lua_error(L, "unfinished capture");
  ...
  if (lua_gettop(L) > 3 || /* extra argument? */
    ...
    do {
      ...
      if ((res = match(L, s1, p, &cap)) != NULL) {
        lua_pushnumber(L, (double)(s1 - s + 1)); /* start */
        lua_pushnumber(L, (double)(res - s)); /* end */
...
static int str_format(lua_State* L)
{
  ...
  while (*strfrmt) {
    if (*strfrmt != '%')
      ...
      case 'g':
      case 'G':
        q = luaL_check_number(L, arg);
        if (Im(q) == 0)
          sprintf(buff, form, Re(q));
        else {
          if (Im(q) > 0)
            sprintf(complexForm, "%s+I*%s", form, form);
          else
            sprintf(complexForm, "%s-I*%s", form, form);
          sprintf(buff, complexForm, Re(q), fabs(Im(q)));
        }
        break;
      case 'q':
